...
def clonar_repositorio(branch="main", repo_url="https://github.com/vidigalbz/SupermercadoWeb", pasta_destino="mercadoDidaticoDigital"):
    try:
        appdata = os.getenv('APPDATA')
        destino = os.path.join(appdata, pasta_destino)
...
def remover_arquivos(pasta_destino="mercadoDidaticoDigital"):
    try:
        appdata = os.getenv('APPDATA')
        destino = os.path.join(appdata, pasta_destino)
        if os.path.exists(destino):
            subprocess.run(["rmdir", "/s", "/q", destino], shell=True)
            return True, "Arquivos removidos com sucesso."
